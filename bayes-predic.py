...
def build_data_set(all_data):
    ...
    for index, row in all_data.iterrows():
        ...
        # 把elo当为评价每个队伍的第一个特征值
        team1_features = [float(team1_elo)]
        team2_features = [float(team2_elo)]
        ...
        team1_features.append(float(team_stats[Wteam]))
        team2_features.append(float(team_stats[Lteam]))
...
if __name__ == "__main__":
    ...
    while k < 911:
        ...
        j = test['客场队名'][k]
        k += 1
        res.append([float(predict_winner(j, i, model=model)[0])])
